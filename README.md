...
The `-s <server>` specifies the address of your Homegear server. The address should be specified as hostname or IP (`homegearpi.local`, `localhost` or `192.168.1.1`), not in URL Syntax (like "http://example.com"). When launching the command the password for the connection will be prompted.
The `-p <port>` is the RPC server port. The default port set up in homegear (0.60) is `2002`, check `/etc/homegear/rpcservers.conf` for more options.
